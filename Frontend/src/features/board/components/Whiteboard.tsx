...
const Whiteboard = () => {
  ...
  // Lấy các actions
  const { initConnection, emitShapeUpdate, emitShapeDelete, emitCursorMove } =
    useBoardStore();
  ...
  // --- 2. XỬ LÝ UPLOAD ẢNH ---
  const { handleFileChange, triggerUpload } = useImageUpload((base64) => {
  ...
  const handleMouseMove = (e: any) => {
    ...
    if (currentShape) {
      ...
      if (updatedShape.type === "arrow")
        updatedShape.points = [
          updatedShape.points[0],
          updatedShape.points[1],
          pos.x,
          pos.y,
        ];
      else if (updatedShape.type === "rect") {
        ...
        updatedShape.radius = Math.sqrt(
          Math.pow(pos.x - updatedShape.x, 2) +
            Math.pow(pos.y - updatedShape.y, 2),
        );
      }
  ...
  const deleteShapes = () => {
    ...
    shapes.forEach((shape) => {
      if (shape.type === "arrow") {
        if (
          (shape.startId && idsToDelete.has(shape.startId)) ||
          (shape.endId && idsToDelete.has(shape.endId))
        ) {
  ...
  return (
    <div className="relative w-full h-screen bg-gray-50 overflow-hidden">
      ...
      {/* UI ZOOM CONTROL */}
        ...
        <div className="absolute bottom-4 left-270 bg-white shadow-md p-2 rounded-lg flex items-center gap-2 z-50 border">
          <Button onClick={() => setStageScale(1)}>Rời khỏi </Button>
        </div>
    </div>
  );
};
